...
export async function handleRequest(req, options) {
  const { dataDir, isDev } = options;
  const url = new URL(req.url);
  const startTime = Date.now();

  // Extract host
  // In dev mode, transform .localhost to production suffix
  let host = url.hostname;
  if (isDev && host.endsWith(".localhost")) {
    const prodSuffix = process.env.RICHHOST_PROD_SUFFIX || ".richcorbs.com";
    host = host.replace(".localhost", prodSuffix);
  }
