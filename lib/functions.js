...
export async function handleFunction(req, ctx) {
  ...
  try {
    ...
    if (ctx.path === "/deploy" && result?.body?.success && result?.body?.host) {
      const host = result.body.host;
      clearFunctionCache(host);
      const sitePath = path.join(DATA_DIR, "sites", host, "current");
      clearSiteConfigCache(sitePath);
    }
...
async function loadFunction(functionPath) {
  if (isDev) {
    try {
      // Resolve symlinks to get real path, then cache-bust
      const fs = await import("fs/promises");
      const realPath = await fs.realpath(functionPath);
      return await import(realPath + `?t=${Date.now()}`);
    } catch (err) {
